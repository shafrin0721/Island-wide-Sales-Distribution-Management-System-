...
// Demo login buttons: auto-create or sign-in demo users for different roles
(function() {
    const demoRoles = {
        'demo-login-customer': { role: 'customer', name: 'Demo Customer' },
        'demo-login-rdc': { role: 'rdc_staff', name: 'Demo RDC Staff' },
        'demo-login-delivery': { role: 'delivery_staff', name: 'Demo Delivery Staff' },
        'demo-login-admin': { role: 'admin', name: 'Demo Admin' }
    };

    Object.entries(demoRoles).forEach(([btnId, config]) => {
        const btn = document.getElementById(btnId);
        if (!btn) return;

        btn.addEventListener('click', async function() {
            const demoEmail = `demo+${config.role}+${Date.now()}@example.com`;
            ...
            try {
                ...
                const newUser = { id: 'demo_' + Date.now(), email: demoEmail, full_name: config.name, role: config.role };
